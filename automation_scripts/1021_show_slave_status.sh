...
#---------------------------------------------------------------------------------------#
# Script Name   : 1010_show_slave_status.sh
# Description   : Script to verify status of slave/replica server
...
if [ $STATE == 'active' ]; then
   STATUS="Show slave status:"; echout "$STATUS"
   STATUS=$($MYSQL -u $USER -S $MYSOCK -e "show slave status\G"); echout "$STATUS"

   STATUS="Show variables like '%gtid%':"; echout "$STATUS"
   STATUS=$($MYSQL -u $USER -S $MYSOCK -e "show variables like '%gtid%';"); echout "$STATUS"
else
   STATUS="$HOSTNAME $(basename $0) - MariaDB Service is DOWN!!! Can't query slave status..."; echout "$STATUS"
