...
#---------------------------------------------------------------------------------------#
# Script Name   : 1009_show_master_status.sh
# Description   : Script to verify status of master/primary server
...
if [ $STATE == 'active' ]; then
   STATUS="Show master status:"; echout "$STATUS"
   STATUS=$($MYSQL -u $USER -S $MYSOCK -e "show master status;"); echout "$STATUS"

   STATUS="Show variables like '%gtid%':"; echout "$STATUS"
   STATUS=$($MYSQL -u $USER -S $MYSOCK -e "show variables like '%gtid%';"); echout "$STATUS"
else
   STATUS="$HOSTNAME $(basename $0) - MariaDB Service is DOWN!!! Can't query master status..."; echout "$STATUS"
