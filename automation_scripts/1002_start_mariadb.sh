...
#---------------------------------------------------------------------------------------#
# Script Name   : 1002_start_mariadb.sh
# Description   : Script to start MariaDB service
...
if [ $STATE == 'active' ]; then
   STATUS="$HOSTNAME $(basename $0) - MariaDB Service is ALREADY running..."; echout "$STATUS"   
else
   sudo $SYSCTL start mariadb
   if [ $? -eq 0 ]; then
      STATUS="$HOSTNAME $(basename $0) - MariaDB Service is STARTED successfully..."; echout "$STATUS"
   else
      STATUS="$HOSTNAME $(basename $0) - MariaDB Service FAILED to start!!!"; echout "$STATUS"
