...
#---------------------------------------------------------------------------------------#
# Script Name   : 1015_list_dbusers.sh
# Description   : Script to list DB users created in MariaDB instance
...
if [ $STATE == 'active' ]; then
   ...
   DBNAME=$($MYCONN -r -B -N -e "select user,host,plugin from mysql.user where is_role!='Y';")
